---
title: "Introducción al aprendizaje automático en R"
author: "Jorge Alexander Fajardo Muñoz"
...
Con esto, podemos hacer un análisis de cual es la relación entre la eficencia de cada carro en cuanto a combustible con cada una de nuestras columnas. Es importante mencionar el significado de cada una de estas columnas.
...
```{r, echo = FALSE}
palabra <- c("Reporte","Función","Modelo","Ecuación","Algoritmo")
definicion <- c("Objeto estático sin poder de predicción","Un objeto que tiene algún tipo de poder de procesamiento, probablemente se encuentra dentro de un modelo","Un objeto complejo que toma un parametro de entrada y da una salida","representación matemática de una función, algunas veces un modelo matemático","
Un conjunto de pasos que se pasan a un modelo para su cálculo o procesamiento.")
tabla2_df <- data.frame(palabra, definicion)
knitr::kable(tabla2_df)
```


No está de más mencionar que hay casos en los que las funciones no necesariamente producen resultados matemáticos, por ejemplo si tenemos una cantidad de datos de forma incorrecta, los datos se podrían "limpiar" mediante una función. Dicho proceso podría modelarse usando un diagrama de flujo en lugar de una ecuación.
...
Es importante estudiar las matemáticas detrás de cada algoritmo y tener los conocimientos estadísticos para entender el funcionamiento interno de un algoritmo de aprendizaje automático, esto para una mejor compresión de cada algoritmo y modelo. De esta forma va a ser menos abrumador el enfoque en el código de R. La siguiente es una lista general de conceptos estadísticos importantes que definen la presición del modelo.

```{r, echo = FALSE}
terminos <- c("Coeficiente de determinación","p-valores","intervalo de confianza") 
defin <- c("A veces aparece como R-cuadrado, esto es lo bien que se ajustan los datos a la línea modelada para análisis de regresión.","
Estas son medidas de validez estadística, donde si su valor p está por debajo de 0.05, es probable que el valor que está examinando sea estadísticamente válido.","
...
tabla3_df <- data.frame(terminos, defin)
knitr::kable(tabla3_df)

```


Estos conceptos podemos usarlos para comprender la diferencia entre un modelo que se ajusta bien a los datos
y uno que encaja mal. Podemos evaluar qué funciones son útiles para que las usemos en nuestro
modelo y podemos determinar la precisión de las respuestas producidas por el modelo. Hay diferentes técnicas de optimización que son una base importante de muchos modelos diferentes que ayudan en obtener unos resultados mas precisos, por ejemplo, como el método de descenso por gradiente.
...
![](C:\Users\jafaj\OneDrive\Escritorio\R\rstudio.png){width=500px}
...
Sin embargo, si quisieramos diferentes enfoques de modelilzación podríamos hacerlo, por ejemplo, probar diferentes funciones para ajustar a los datos, porque si intentamos ajustar un motor teórico
de 50.000 pulgadas cúbicas, la eficiencia del combustible se vuelve negativa!. Estos modelos de regresión en R serán estudiados más adelante.
...
### Metodos supervisados de Clustering
...
Cuando hay muy pocos clusters y los datos están underfit (infrajuste), no hay una buena manera de determinar la estructura. Mientras que cuando hay demasiados clusters se tienen los problemas opuestos, es decir, hay demasiada estructura para darle sentido simplemente.
...
La figura muestra un árbol de inferencia condicional. Estamos dibujando la eficiencia de combustible (mpg), pero usamos todas las funciones del conjunto de datos para crear el modelo en lugar de solo uno; por lo tanto el mpg ~. llamado en la función ctree (). 

La salida es una distribución (en forma de diagrama de caja y bigotes) de la eficiencia del combustible como función de las principales características que influyen en él. La función ctree llama a ciertos métodos para
resolver estos; de esta manera, no tienes un montón de ramas en el árbol obstruyan la vista.
...
Cuanto más cercano esté el valor p a 0,05 o más, menos útil o relevante es. En este caso, un valor p de casi exactamente 0 es muy bueno. Asimismo, puedes ver cuántos puntos de datos componen cada clase en la parte inferior del árbol.
...
Los algoritmos de bosque aleatorio son mucho más difíciles de mostrar en una visualización;
Sin embargo, podemos mostrar fácilmente cómo evoluciona el error en el modelo según la cantidad de árboles que introducimos en el modelo.

La figura anterior muestra la restricción del error en un algoritmo de bosque aleatorio con 1,000
árboles utilizados. Puede ver que el error disminuye con más árboles que uso, y es mínimo alrededor del área de n = 500 árboles.
...
Podemos definir una red neuronal simple como una que toma dos entradas, calcula la
función AND y nos da un resultado. Esto puede representarse por medio de una figura donde tenemos capas y nodos. Las capas son secciones verticales de lo visual y los nodos son los puntos de cálculo dentro de cada capa.
...
En el caso de la función AND, usaremos valores numéricos que pasamos por una función de clasificación para dar un valor de 1 para VERDADERO y 0 para FALSO. Podemos hacer esto usando el función sigmoidea:
...
Comenzamos con las entradas X1, X2 y el nodo de polarización que es solo una constante aditiva. Calculamos todos estos en el círculo vacío, esto es un nodo de cálculo. El cálculo que realizamos es poner todas estas cosas en una función de activación, que casi siempre es una función sigmoidea. La salida de la función sigmoidea es la resultado de la red neuronal.

![](C:\Users\jafaj\OneDrive\Escritorio\R\red.png)
...
* Capa oculta o de "cálculo" (The hidden layer)
...
Las máquinas vectoriales de soporte, o SVM, son otro algoritmo que puede utilizar tanto para
regresión y clasificación. A menudo, se presenta como un corolario más simple o más rápido que una red neuronal.Funcionan de manera similar a regresión logística aunque involucra mas complejidades estadísticas, lo cual se estudiará mas adelante. La idea es tomar datos y probarlos para encontrar un plano o una línea que pueda separar los datos en diferentes clases.
