---
title: 'Capítulo 5: Redes neuronales'
author: "IPREA"
...
En el Capítulo 2, se observo el desarrollo de una puerta $AND$. Una puerta $AND$ tiene la siguiente lógica.
...
## Construyendo una red neuronal simple usando R
...
$$z = -11.86048 + 7.75382 Var_1 + 7.75382 Var_2$$
...
$$AND = \frac{1}{1+ e^{-(-11.86048 + 7.75382 Var_1 + 7.75382 Var_2)}}$$
...
$\begin{align*}
AND &= 19.4 + 7.5 \cdot Var_1 + 7.6 \cdot Var_2 + 7.6 \cdot Var_3 \\
OR &= 10.3 + 22.3\cdot Var_1 + 21.8 \cdot Var_2 + 21.9 \cdot Var_3 
\end{align*}$
...
\begin{equation*}
H_1 = 8.57 + 3.6 \cdot Var_1 -3.5 \cdot Var_2 -3.6 \cdot Var_3
\end{equation*}

Que luego pasaremos a través de la función logística. Entonces:

\begin{equation*}
g(H_1) = \frac{1}{1+e^{-(8.57 + 3.6 \cdot Var_1 -3.5 \cdot Var_2 -3.6 \cdot Var_3)}}
\end{equation*}
...
\begin{align*}
f_1 &= 13.64 + 13.97\cdot Var_1 + 14.9\cdot Var_2 + 14.27\cdot Var_3 \\
f_2 &= -7.95 + 3.24\cdot Var_1 + 3.15\cdot Var_2 + 3.29\cdot Var_3 \\
f_3 &= -5.83 - 1.94\cdot f_1 + 14.09\cdot f_2 \\
AND &= \frac{1}{1-e^{-(-5.83 - 1.94\cdot f_1 + 14.09\cdot f_2)}}
\end{align*}
...
```
Intentar usar una red neuronal sin capas ocultas es un gran error. Al observar el resultado de la función de `prediction()`, puede ver que la red neuronal piensa que para un escenario dado, como $xor(0,0)$, la respuesta es $0.5 ± 2.77$. Tener un error que es mucho más alto que el nivel de granularidad para el que está tratando de encontrar la respuesta indica que este no es el mejor método que puede usar.
